...
/// <summary>
/// Класс с юнит-тестами для поликлиники.
/// </summary>
public class PolyclinicTests
{
    ...
    /// <summary>
    /// Тест метода, возвращающего информацию о всех врачах, стаж работы которых не меньше 10 лет.
    /// </summary>
    [Fact]
    public void GetDoctorsWithExperienceMoreThan10Years_ReturnsCorrectDoctors()
    {
        ...
        {
            var expectedInfo = $"Врач: {doctor.FullName}, Специализация: {doctor.Specialization}, Стаж: {doctor.Experience} лет";
            Assert.Contains(expectedInfo, result);
        }
    }

    /// <summary>
    /// Параметризованный тест метода, возвращающего список пациентов, записанных на прием к указанному врачу, упорядоченных по ФИО.
    /// </summary>
    ...
    /// <summary>
    /// Тест метода, возвращающего информацию о здоровых на настоящий момент пациентах.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Тест метода, возвращающего информацию о количестве приемов пациентов по врачам за последний месяц.
    /// </summary>
    [Fact]
    public void GetAppointmentsCountByDoctorLastMonth_ReturnsCorrectCounts()
    {
        ...
        foreach (var appointment in appointmentsByDoctor)
        {
            ...
            var expectedInfo = $"Врач: {doctor.FullName}, Количество приемов: {appointment.Count}";
            Assert.Contains(expectedInfo, result);
        }
    }

    /// <summary>
    /// Тест метода, возвращающего информацию о топ 5 наиболее распространенных заболеваниях среди пациентов.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Тест метода, возвращающего информацию о пациентах старше 30 лет, которые записаны на прием к нескольким врачам, упорядоченных по дате рождения.
    /// </summary>
    [Fact]
